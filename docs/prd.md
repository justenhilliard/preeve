...
**Important scope note:** `preferred_fits` is now used by the v1 verdict engine
when the vision LLM can detect an item's fit/silhouette. The fit signal is
best-effort and only participates when both the user has set fit preferences and
the scan produced a valid `visualAttributes.fit` value.
...
- **FR-3.6** — Backend may also extract a small structured set of visual
  attributes from the photo using a vision LLM: garment type, primary color,
  secondary colors, pattern/material, and fit/silhouette. This does not expand
  FR-3.2's locked category/color taxonomies. The nested `fit` value is a
  best-effort input to FR-4's verdict matrix when visually apparent; other
  visual attributes remain display/context metadata. User-facing closet context
  is generated by deterministic Preeve code that compares the item's effective
  category/color with the user's saved wardrobe, not by LLM-written advice.
- **FR-3.7** — Backend runs hosted background removal on the compressed upload
  before classification/extraction to reduce skin, floor, wall, and other
  background color interference. This changes only the model input bytes; it
  does not change FR-3.2's taxonomies, CLIP prompts, stored wardrobe photo, or
  FR-4's verdict engine.
...
- **FR-4.1** — System compares the detected (or user-corrected)
  category/color, detected fit when available, and category-implied formality
  against the user's saved preferences using deterministic, priority-ordered
  rules — **not** a weighted or numeric scoring model. Similarity/embedding-
  based scoring is explicitly roadmap, not v1.
...
Signals are applicable only when the user set that preference and the item has a
value to compare. Color applies when `preferredColors` is non-empty. Fit applies
when `preferredFits` is non-empty and `visualAttributes.fit` is not null.
Formality applies when `formalityPreference` is set.

| Priority | Condition | Verdict | Rationale template |
|---|---|---|---|
| 0 | No signal is applicable | **Maybe** | `"Set your style preferences to get a personalized verdict."` |
| 1 | Every applicable signal fails | **Skip** | Name each failed signal. |
| 2 | Every applicable signal matches | **Buy** | Name each matched signal. |
| 3 | Some applicable signals match and some fail | **Maybe** | Name both the match and tension. |

**`CATEGORY_FORMALITY_MAP`** (the coarse, deterministic stand-in for item-level
formality detection):
...
**Known, accepted limitation:** `outerwear`/`shoes`/`accessory` are mapped as
formality-compatible with everything. The category taxonomy is still too coarse
to discriminate a formal blazer from a casual jacket without true item-level
formality detection. Formality only meaningfully fails for `top`/`bottom` with a
`formal` preference and `dress` with an `athleisure` or `casual` preference.
...
- **Cost:** Entire stack (hosting, database, storage, inference API) should
  stay within free or low-cost tiers appropriate for low, unpredictable
  traffic. The scan path now has two external model dependencies — CLIP
  classification plus a small vision-LLM structured extraction call — so
  per-scan cost and latency should be monitored together.
...
- Live capture-quality guidance on the capture screen — client-side brightness/blur checks on the canvas frame (e.g. average luminance for "too dim," edge-variance for "too blurry") to catch unusable photos before they're sent to CLIP, saving a wasted classification call. Evaluate after the real scan pipeline (`POST /api/items/scan`) is live and there's real data on how often bad photos cause bad classifications.
- Live object-framing guidance ("center the item") — would require on-device object detection (e.g. TensorFlow.js) to know where the item is in the live camera feed, not just a static hint. Meaningfully bigger scope than the brightness/blur checks above; not worth it unless CLIP classification accuracy is actually being limited by framing in practice.
- ~~Richer verdict rationale~~ — **done.** The verdict engine now composes color, fit, and formality into one multi-clause deterministic rationale (FR-4.1/FR-4.3), and the item detail screen adds a structured signal checklist, a fit-aware styling tip, wardrobe-composition insight, and personalized pairing suggestions drawn from the user's own saved wardrobe. An LLM-elaborated rationale was considered and explicitly rejected — the richness comes from composing more real, structured signals, not from delegating the actual reasoning to a model, keeping the engine deterministic, testable, and free of per-request AI cost.
- In-app feedback mechanism — a real feedback form (not just a mailto link) surfaced somewhere in the authenticated app, once there are actual users generating verdicts to have opinions about. Deferred rather than built into the pre-launch landing page, which has no users yet to collect feedback from.
