...
- Richer verdict rationale — the v1 rationale (FR-4.3) is deliberately a single short template sentence, to keep the verdict engine deterministic, testable, and free of per-request AI cost (FR-4.1). A future pass could generate a fuller explanation (e.g. an LLM elaborating on the rule that fired, or templates that reference more of the user's full preference profile for transparency) once the verdict screen has more surrounding context (pairing suggestions, wardrobe history) to draw on.
