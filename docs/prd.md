...
- **FR-3.6** — Backend may also extract a small structured set of visual
  ...
  secondary colors, and pattern/material. This is perception metadata only; it
  does not expand FR-3.2's locked category/color taxonomies and is not used by
  FR-4's verdict engine. User-facing closet context is generated by
  deterministic Preeve code that compares the item's effective category/color
  with the user's saved wardrobe, not by LLM-written advice.
